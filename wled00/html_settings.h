...
var locip,d=document,loc=!1,locproto="http:",scanLoops=0,preScanSSID="";function gId(t){return d.getElementById(t)}function cE(t){return d.createElement(t)}function H(){window.open("https://kno.wled.ge/features/settings/#wifi-settings")}function B(){window.open(getURL("/settings"),"_self")}function N(){const t=gId("scan");t.disabled=!0,t.textContent="Scanning...",t.style.background="#fff",t.style.color="#000",fetch(getURL("/json/net")).then(t=>t.json()).then(t=>Object.assign({},{networks:[]},t).networks.sort((t,e)=>e.rssi-t.rssi).reduce((t,e)=>(t.some(t=>t.ssid===e.ssid)||t.push(e),t),[])).then(e=>{if(0===e.length&&scanLoops<10)return scanLoops++,void setTimeout(N,1e3);scanLoops=0;let o=gId("CS");if(o){let t=cE("select");t.setAttribute("id","CS"),t.setAttribute("name","CS"),t.setAttribute("onchange","T()"),preScanSSID=o.value;for(let e=0;e<t.children.length;e++)t.removeChild(t.children[e]);for(let n=0;n<e.length;n++){const s=cE("option");s.setAttribute("value",e[n].ssid),s.textContent=`${e[n].ssid} (${e[n].rssi} dBm)`,e[n].ssid===o.value&&s.setAttribute("selected","selected"),t.appendChild(s)}const n=cE("option");n.setAttribute("value","!Cs"),n.textContent="Other network...",t.appendChild(n),o.replaceWith(t)}t.disabled=!1,t.textContent="Scan",t.style.background="#000",t.style.color="#fff"})}function T(){let t=gId("CS");if(!t||"!Cs"!=t.value)return;let e=cE("input");e.type="text",e.id="CS",e.name="CS",e.setAttribute("maxlength",32),e.value=preScanSSID,t.replaceWith(e)}function loadJS(t,e=!0){let o=cE("script");o.setAttribute("src",t),o.setAttribute("type","text/javascript"),o.setAttribute("async",e),d.body.appendChild(o),o.addEventListener("load",()=>{GetV()}),o.addEventListener("error",t=>{console.log("Error on loading file",t),alert("Loading of configuration script failed.\nIncomplete page data!")})}function S(){let t=window.location;if("file:"==t.protocol)loc=!0,(locip=localStorage.getItem("locIp"))||(locip=prompt("File Mode. Please enter WLED IP!"),localStorage.setItem("locIp",locip));else{let e=t.pathname,o=e.slice(1,e.endsWith("/")?-1:void 0).split("/");o.length>2&&(locproto=t.protocol,loc=!0,locip=t.hostname+(t.port?":"+t.port:"")+"/"+o[0])}loadJS(getURL("/settings/s.js?p=1"),!1),loc&&(d.Sf.action=getURL("/settings/wifi"))}function getURL(t){return(loc?locproto+"//"+locip:"")+t}
</script><style>
...
class="s" min="0" max="255" required required value="0"> . <input name="I1" 
type="number" class="s" min="0" max="255" required required value="0"> . <input 
name="I2" type="number" class="s" min="0" max="255" required required value="0">
 . <input name="I3" type="number" class="s" min="0" max="255" required required 
value="0"><br>Static gateway:<br><input name="G0" type="number" class="s" 
min="0" max="255"> . <input name="G1" type="number" class="s" min="0" max="255">
 . <input name="G2" type="number" class="s" min="0" max="255"> . <input 
name="G3" type="number" class="s" min="0" max="255"><br>Static subnet mask:<br>
<input name="S0" type="number" class="s" min="0" max="255"> . <input name="S1" 
